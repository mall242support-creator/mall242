...
const FloatingWhatsApp = () => {
  ...
  const COMPANY_NUMBER = '12425551234';
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 transition-all duration-300 transform ${
      ...
      >
        ...
        <div className="relative w-14 h-14 md:w-16 md:h-16 bg-green-500 rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 transition-all hover:scale-110">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden animate-slideUp">
          <div className="bg-green-500 p-4 text-white">
            ...
            <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-white hover:text-gray-200">
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            <div className="mb-4">
              <div className="bg-gray-100 rounded-lg p-3 inline-block max-w-[85%]">
                <p className="text-sm text-gray-700">👋 Hi there! Welcome to Mall242 Support. How can we help you today?</p>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {quickQuestions.map((q, idx) => (
                  <button key={idx} onClick={() => handleQuickQuestion(q.message)} className="w-full text-left flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg">
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="+1-242-555-0123" className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" />
                <button onClick={handlePhoneInquiry} disabled={!phoneNumber.trim()} className="px-3 py-2 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 disabled:opacity-50">Send</button>
              </div>
            </div>

            <div>
              <p className="text-xs text-gray-500 mb-2">TYPE YOUR MESSAGE</p>
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows="3" placeholder="Write your message here..." className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 resize-none" />
              <button onClick={handleCustomMessage} disabled={!message.trim()} className="w-full mt-2 py-2 bg-green-500 text-white rounded-lg text-sm font-semibold hover:bg-green-600 disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div className="bg-gray-50 p-3 text-center border-t border-gray-200">
            <p className="text-xs text-gray-400">Our support team is available 9 AM - 6 PM, Monday to Saturday</p>
          </div>
        </div>
      )}
    </div>
